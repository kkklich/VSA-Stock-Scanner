...
class TestGetCompanies:
    ...
    def test_kghm_is_present(self) -> None:
        ...
        assert kghm["name"] == "KGHM Polska Miedź"
        assert kghm["sector"] == "Mining"
...
class TestGetRanking:
    ...
    def test_ranking_result_is_cached(self) -> None:
        ...
        # 30 tickers × 1 call each on first load, then 0 calls on cache hit.
        first_load_calls = calls["count"]
        assert first_load_calls == 30  # one stooq call per company
